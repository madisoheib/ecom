@if ($paginator->hasPages())
    <nav role="navigation" aria-label="Pagination Navigation" class="flex items-center justify-between">
        <div class="flex justify-between flex-1 sm:hidden">
            ...
            @else
                <a href="{{ $paginator->previousPageUrl() }}" class="relative inline-flex items-center px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 leading-5 rounded-md hover:text-gray-500 focus:outline-none focus:ring focus:border-gray-400 active:bg-gray-100 active:text-gray-700 transition ease-in-out duration-150"
                   style="focus:ring-color: rgba(var(--color-secondary-rgb), 0.3);"
                    {!! __('pagination.previous') !!}
                </a>
            @endif
        ...
        <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-center">
            <div>
                <span class="relative z-0 inline-flex shadow-sm rounded-lg">
                    ...
                    @else
                        <a href="{{ $paginator->previousPageUrl() }}" rel="prev" class="relative inline-flex items-center px-3 py-2 text-sm font-medium text-gray-500 bg-white border border-gray-300 rounded-l-lg leading-5 hover:text-gray-400 focus:z-10 focus:outline-none focus:ring focus:border-gray-400 active:bg-gray-100 active:text-gray-500 transition ease-in-out duration-150" 
                               style="focus:ring-color: rgba(var(--color-secondary-rgb), 0.3);" 
                                ...
                                @if ($page == $paginator->currentPage())
                                    <span aria-current="page">
                                        <span class="relative inline-flex items-center px-4 py-2 -ml-px text-sm font-bold cursor-default leading-5" 
                                              style="background-color: var(--color-secondary); color: var(--color-primary); border: 1px solid var(--color-secondary);">{{ $page }}</span>
                                    </span>
                                @else
                                    <a href="{{ $url }}" class="relative inline-flex items-center px-4 py-2 -ml-px text-sm font-medium text-gray-700 bg-white border border-gray-300 leading-5 hover:text-gray-500 focus:z-10 focus:outline-none focus:ring focus:border-gray-400 active:bg-gray-100 active:text-gray-700 transition ease-in-out duration-150" 
                                       style="focus:ring-color: rgba(var(--color-secondary-rgb), 0.3);"
                                       onmouseover="this.style.backgroundColor='rgba(var(--color-secondary-rgb), 0.1)';" 
